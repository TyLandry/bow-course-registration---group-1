...
<div className="flex justify-evenly items-start gap-80 mt-20">
  ...
  <div className="flex flex-col">
    ...
    <input
      ...
      className="border border-gray-400 rounded-md px-3 py-2 w-[500px] focus:outline-none focus:ring-2 focus:ring-blue-500 mt-4"
    />
    <label className="block mt-2 mb-4 font-semibold">Select Country</label>
        <div className="flex gap-3">
            {countryOptions.map((country) => (
            <button key={country} onClick={() => settSelectedCountry(country)} className={`px-4 py-2 text-sm rounded-md ${selectedCountry === country ? 'bg-1 text-white' : 'bg-gray-200 text-gray-700 hover:bg-gray-300'}`}>
                {country}
            </button>
            ))}
        </div>
    <div className="flex gap-4 mt-6">
        <button className="btn-primary-fill py-2 px-4 text-sm">Sign Up</button>
